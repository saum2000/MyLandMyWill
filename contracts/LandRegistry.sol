...
// string memory _name, uint _age, string memory _designation
    ...
    function verifySeller(address _sellerAddress) _onlyGovtOfficial public{
        SellerVerification[_sellerAddress] = true;
    }

    function rejectSeller(address _sellerAddress) _onlyGovtOfficial public{
        SellerRejection[_sellerAddress] = true;
    }

    function verifyBuyer(address _buyerAddress) _onlyGovtOfficial public{
        BuyerVerification[_buyerAddress] = true;
    }
    ...
    function createWill(uint _landId, Buyer memory heir) _onlyRegisteredSeller  public {
        require(LandOwner[_landId] == msg.sender, "msg sender is not the owner of the property");
        wills[_landId] = heir;
    }

    function deleteWill (uint _landId) _onlyRegisteredSeller public {
        require(LandOwner[_landId] == msg.sender, "msg sender is not the owner of the property");
        delete wills[_landId];
    }

    function transferWillProperty (uint _landId) _onlyGovtOfficial public {
